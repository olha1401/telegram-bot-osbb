...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Привіт! Я бот, який допомагає передавати питання Правлінню ОСББ. Оберіть одну з опцій:",
        reply_markup=markup
    )

async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Щоб задати питання Правлінню ОСББ, просто напишіть мені повідомлення. "
        "Я відправлю його їм, після чого ви отримаєте відповідь у цей чат."
    )
...
async def admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Загалом питань: {total}\n"
        f"Очікує відповіді: {waiting}\n"
        f"Відповіді надіслані: {answered}"
    )
...
async def handle_user_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Ваше питання передано Правлінню. Очікуйте на відповідь.")
...
# Создаем Application для telegram.ext
...
# Создаем отдельный event loop для очереди обновлений
...
if __name__ == "__main__":
    # Запускаем Flask-сервер, слушающий 0.0.0.0:5000
